...
**Networking / env:** nginx :443 → Node/pm2 → Azure-hosted PostgreSQL. Trust the current repo-root `deploy.sh` and `DEPLOYMENT.md` for the authoritative port and nginx upstream. `.env` (repo root) needs `DATABASE_URL` (no quotes); set `NODE_ENV=production` (pm2 `run start` handles this). Keep secrets in ONE place (the `.env`), not also `~/.bashrc`.
