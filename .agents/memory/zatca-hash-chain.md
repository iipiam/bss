...
- Every invoice source must also enter through the shared per-restaurant PostgreSQL advisory-lock orchestration before the process-local signing lock.
...
- **How to apply:** never call the signing service directly from a new order channel; use the shared idempotent orchestration so multiple app processes serialize the tenant's full EGS chain.
