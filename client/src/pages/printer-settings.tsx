...
import { Plus, Search, Edit, Trash2, Printer, Star, Wifi, Usb, Bluetooth, CheckCircle } from "lucide-react";
...
function PrinterCard({ printer, onEdit, onDelete, onSetDefault, t }: PrinterCardProps) {
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="space-y-1">
              <div className="flex items-center gap-2">
                <h3 className="font-semibold" data-testid={`text-printer-name-${printer.id}`}>{printer.name}</h3>
                {printer.isDefault && (
              ...
              <p className="text-sm text-muted-foreground" data-testid={`text-printer-brand-${printer.id}`}>
                {printer.brand}
              </p>
              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                ...
                {printer.connectionType === "network" && printer.ipAddress && (
                  <span className="text-xs bg-muted px-2 py-0.5 rounded" data-testid={`text-printer-ip-${printer.id}`}>
                    {printer.ipAddress}
                  </span>
                )}
              </div>
...
export default function PrinterSettings() {
  ...
  const printerFormSchema = z.object({
    ...
    message: t.enterIpAddress || "IP address is required for network printers",
    path: ["ipAddress"],
  });
  ...
  const updatePrinterMutation = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: PrinterFormValues }) => {
  ...
  const filteredPrinters = printers.filter((printer) =>
    ...
    (printer.brand && printer.brand.toLowerCase().includes(searchQuery.toLowerCase()))
  );
        ...
        <CardContent>
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center">
              ...
              <h3 className="text-lg font-semibold mb-1">{t.noPrintersFound}</h3>
              <p className="text-sm text-muted-foreground mb-4">
                {t.addFirstPrinter}
              </p>
              <Button onClick={() => setOpen(true)} data-testid="button-add-first-printer">
                <Plus className="mr-2 h-4 w-4" />
                {t.addPrinter}
              </Button>
            </div>
