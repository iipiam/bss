...
- **Granular Permission Enforcement System**: Comprehensive role-based access control enforcing 15+ granular permissions at both backend and frontend. Backend middleware (`requirePermission`) guards all critical routes (inventory, menu, recipes, branches, users, POS, orders, customers, sales, reports, procurement, bills, working hours). Frontend `usePermissions` hook and permission-aware sidebar automatically filter navigation based on user permissions. Admins bypass all permission checks. Session management includes user permissions for real-time enforcement.
