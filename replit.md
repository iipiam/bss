...
- **Internationalization** (Nov 23, 2025): Comprehensive multi-language support for **8 languages** (English, Arabic, Hindi, Urdu, Spanish, Tagalog, French, Indonesian) with full RTL support for Arabic/Urdu. Implemented 1000+ translation keys covering all user-facing text including form validation messages, toast notifications, export labels, and error handling. Translation system uses LanguageContext with useLanguage() hook. Language preferences persist in localStorage and sync with backend settings for authenticated users. All Zod form schemas use translation keys (restructured to component-scoped or factory functions). PaymentTest form reacts to language changes via useEffect. LanguageToLocaleCode mapping updated for all 8 languages. **Turkish and Swahili removed** from supported languages (user request). **Language validation** added to LanguageContext to automatically fallback to English for invalid stored languages, preventing crashes. Known limitations: Export payload data structures (object keys in PDF/Excel exports) and some dynamic server-fed labels remain in English (acceptable as technical data).
...
- **Multi-tenant Architecture**: Complete data isolation using `restaurantId` for both restaurant and factory business types.
  ...
  - **Recent Bug Fixes (Nov 17-22, 2025)**: 
    ...
    - **Fixed critical login crash from missing translations** (Nov 23): Root cause identified - Turkish and Swahili were listed in Language type and supportedLanguages array but did not exist in the translations object, causing undefined access to businessManagementSystem. Fixed by: (1) Removing Turkish and Swahili from Language type and supportedLanguages array (reducing from 10 to 8 languages); (2) Completing French and Indonesian translations (added 900+ missing keys using English as placeholder); (3) Adding missing authentication keys (emailAddress, enterYourEmail, sendResetLink, etc.) to Arabic, Hindi, Urdu, Spanish, Tagalog; (4) Removing duplicate keys (noInvoicesFound, noPDF) from Arabic section; (5) Adding language validation in LanguageContext to automatically fallback to English when invalid languages (like Turkish/Swahili) are stored in localStorage. Login page now loads successfully in all 8 supported languages without crashes. E2E testing verified all languages work correctly.
