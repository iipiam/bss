...
- **Financial Management**: Delivery app cost calculation, PDF export for financial statements, Excel import/export. **Updated Dec 07, 2025**: Added Total Inventory Value to Expenses tab in Financial Statements - displays inventory value card, includes inventory in total expenses calculation, and shows inventory in expenses breakdown summary. Added BEP (Break Even Point) analysis card showing break even point calculation, progress bar, fixed/variable costs breakdown, contribution margin, and profit/loss status. Added manual delivery profitability entry system - new delivery_profitability table stores per-app, per-month financial data (orders, sales, revenue, commission, banking, subsidy, POS fees, net earnings) with tabbed UI in delivery apps page for CRUD operations. Delivery breakdown analytics now aggregate from manual entries instead of calculating from orders.
