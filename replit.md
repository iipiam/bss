...
- **IT Business Management**: IT accounts can view all client subscriptions, contact details, subscription invoices, and generate ZATCA-compliant VAT statements (15% rate). Features include client list with subscription status, invoice browser with date filters, VAT summary calculator, and PDF/Excel export for VAT statements. Access via `/business-management` route. **Added Dec 04, 2025**.
