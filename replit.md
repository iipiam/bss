...
- **Ticketing System & IT Support**: Comprehensive ticket management with status, priority, real-time chat interface, and activity logging. Role-based access control and auto-refresh for messages. **IT Dashboard Account Type Isolation (Nov 2025)**: Complete separation of IT Dashboard from client accounts through dual account type system. Login flow includes account type selection (Client vs IT). Session stores accountType preference. Frontend sidebar filters IT Dashboard visibility (only shown for IT account type). Backend middleware (`requireITAccount`) protects all `/api/it/*` routes, returning 403 Forbidden for Client accounts. **Cross-Tenant IT Access (Nov 2025)**: IT accounts have cross-tenant visibility across all restaurants/businesses. All IT Dashboard storage methods (`getITAnalytics`, `getITStaff`, `getWorkloadDistribution`, `getCategoryBreakdown`, `getTicketTrends`, `assignTicket`, `getAllActiveTicketsForIT`) support optional restaurantId parameter. When restaurantId is omitted, queries return data from all tenants. IT routes pass undefined/null for restaurantId to enable global ticket management. This allows IT support staff to manage tickets, view analytics, and assign work across all customer accounts from a single dashboard. E2E tested with comprehensive verification of sidebar filtering, API protection, page access controls, and cross-tenant data access. **Real-Time Ticket Tracking (Nov 2025)**: WebSocket-based real-time updates for support tickets with instant notifications. System broadcasts `ticket:created`, `ticket:updated`, and `ticket:message` events across all connected clients within the same restaurant. NotificationContext automatically invalidates query cache and displays toast notifications with audio alerts for ticket lifecycle events. Direct message dialog in ticket detail page auto-refreshes messages every 5 seconds as fallback to WebSocket updates. All ticket lists, details, and messages update in real-time without manual refresh, providing seamless collaboration between users and IT support staff. **IT-Only Status Control (Nov 2025)**: Ticket status updates to 'in-progress', 'resolved', and 'closed' are restricted to IT accounts only. Backend validation (403 Forbidden) enforces this restriction. Client accounts can only create tickets with 'open' status and communicate via messages. Status update buttons only visible to IT accounts in ticket detail page, ensuring proper workflow where only IT support staff can progress tickets through lifecycle stages.
