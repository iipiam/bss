...
- **Blogger QR Discount (Marketing/POS)**: Each blogger profile has an optional customer discount (percent or fixed SAR, set in the Commission Tiers dialog, validated as enum + >=0, percent capped at 100). POS QR scan of `BSS-BL:<id>` returns the discount; checkout sends `discountCode: BSS-BL:<id>` and the server re-resolves the blogger (tenant-scoped) and recomputes `discountAmount` from items, ignoring any client-sent amount. The bilingual receipt shows a Discount row when discountAmount > 0.
