...
- **Multi-tenant Architecture**: Complete data isolation using `restaurantId` for both restaurant and factory business types.
  ...
  - **Recent Bug Fixes (Nov 17-20, 2025)**: 
    ...
    - **Added Performance tracking for IT accounts** (Nov 20): New Performance tab in System section for IT accounts tracks sales per restaurant/client account. Shows restaurant name, business type, total sales, total orders, average order value, active staff (current snapshot with tooltip), and last activity. Includes date range filtering (7/30/60/90 days) and search by restaurant name. SQL query uses scalar subquery for activeUsersCount to avoid fan-out/double-counting issues.
