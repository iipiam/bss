...
- **Granular Permission Enforcement System**: Comprehensive role-based access control enforcing 18 granular permissions at both backend and frontend. All permissions defined in `shared/permissions.ts` (ALL_PERMISSIONS array, PermissionSet type, DEFAULT_EMPLOYEE_PERMISSIONS). Backend middleware (`requirePermission`) guards all critical routes. Frontend `usePermissions` hook and permission-aware sidebar automatically filter navigation. Admins bypass all permission checks. Employee form uses DEFAULT_EMPLOYEE_PERMISSIONS to prevent frontend/backend drift. Session management includes user permissions for real-time enforcement. **Known improvement opportunities**: 1) Add type safety to employee form (replace `any` with PermissionSet), 2) Add backend validation to reject unknown permission keys, 3) Add regression tests for permission schema.
